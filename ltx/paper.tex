...
This section details a Markov Chain Monte Carlo (MCMC) sampler used to perform inference, which provides another method to carry out unsupervised detection and tracking. It operates by iteratively sampling from conditional distributions of elements in the joint probability distribution--i.e. implementing Gibbs sampling--in order to approximate the posterior distribution of the cluster parameters given the data.    used to perform inference, which makes use of the Metropolis-Hastings (M-H) algorithm to sample from a few distributions.
...
\end{cases}
\end{equation}
\willie{$d_{i}$ is actually a time; product needs to actually go up to last observation at time $d_{i}$} where the distribution under the product $P(c_{i'} | m_{i'-1}, c_{1:i'-1})$ is given in, and is taken under the situation where $c = k$, ie etc
...
The Metropolis-Hastings algorithm is used to gain samples from the posterior distribution for the auxiliary variables; it involves construction of a markov chain with the desired 
